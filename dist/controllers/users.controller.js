...
export class UserController {
    static async update(c) {
        try {
            ...
            if (!record)
                return c.json(ResFormmater.failed("User tidak ditemukan", 404), 404);
            let imageUrl;
            if (avatar) {
            ...
            if (!user)
                return c.json(ResFormmater.failed("User tidak ditemukan"), 404);
    ...
    static async getAll(c) {
        try {
            ...
            if (!response)
                return c.json(ResFormmater.success([], "User tidak ditemukan", 200), 200);
    ...
    static async delete(c) {
        try {
            ...
            if (!userRole.isAdmin)
                return c.json(ResFormmater.failed("Hanya admin yang bisa hapus", 400), 400);
            const id = Number(c.req.param("id"));
            if (!id)
