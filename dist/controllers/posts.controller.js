...
export class PostsController {
    static async create(c) {
        try {
            ...
            if (!userAuth)
                return c.json(ResFormmater.failed("Uanuthorization"), 404);
            const formData = await c.req.formData();
            if (!formData)
                return c.json(ResFormmater.failed("Harus mengirim form data"));
            const title = formData.get("title");
            const content = formData.get("content");
            const image = formData.get("image");
            if (!title || !content || !image)
                return c.json(ResFormmater.failed("Harus terdapat title, content dan image"), 400);
            if (!(image instanceof File))
                return c.json(ResFormmater.failed("Image harus berupa file yang valid", 400), 400);
    ...
    static async update(c) {
        try {
            const userAuth = c.get("user");
            if (!userAuth)
                return c.json(ResFormmater.failed("Unuthorization"), 404);
            const id = Number(c.req.param("id"));
            const record = await PostsModel.getById(id);
            if (!record)
                return c.json(ResFormmater.failed("Postingan tidak ditemukan", 404), 404);
            const formData = await c.req.formData();
            const title = formData.get("title");
            const content = formData.get("content");
            const image = formData.get("image");
            if (!title && !image && !content)
                return c.json(ResFormmater.failed("Harus update setidaknya 1 data"), 400);
            let imageUrl;
            if (image) {
            ...
            if (!response)
                return c.json(ResFormmater.success([], "Postingan tidak ditemukan", 200), 200);
    ...
    static async getByAuthorId(c) {
        try {
            ...
            if (isNaN(authorId))
                return c.json(ResFormmater.failed("Invalid user id", 400), 400);
            const response = await PostsModel.getByAuthorId(authorId);
            if (!response || response.length === 0)
                return c.json(ResFormmater.success([], "Posts not found", 200), 200);
            return c.json(ResFormmater.success(response, "Get posts by author id successfully"), 200);
        }
        catch (err) {
    ...
    static async getById(c) {
        try {
            ...
            if (!response)
                return c.json(ResFormmater.failed("Posts not found", 404), 404);
    ...
    static async delete(c) {
        try {
            ...
            if (!user)
                return c.json(ResFormmater.failed("Unuthorization", 403), 403);
            const id = Number(c.req.param("id"));
            const record = await PostsModel.getById(id);
            if (!record)
                return c.json(ResFormmater.failed("Posts tidak ditemukan", 404), 404);
            const imageId = record.imageId;
            if (imageId)
                await SupabaseDelete(imageId);
            const response = await PostsModel.delete(id, user.userId, user.isAdmin);
            if (!response)
                return c.json(ResFormmater.failed("Gagal menghapus postingan", 404), 404);
