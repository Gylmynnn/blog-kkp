...
export class PostsController {
    static async create(c) {
        try {
            ...
            return c.json(ResFormmater.success(response, "Berhasil membuat postingan", 201), 201);
        }
        catch (err) {
    ...
    static async update(c) {
        try {
            const userAuth = c.get("user");
            if (!userAuth)
                return c.json(ResFormmater.failed("Unauthorized"), 404);
            const id = Number(c.req.param("id"));
            if (!id)
                return c.json(ResFormmater.failed("Id tidak ditemukan atau tidak valid"));
    ...
    static async getByAuthorId(c) {
        try {
            ...
            if (isNaN(authorId))
                return c.json(ResFormmater.failed("Author id tidak valid", 400), 400);
    ...
    static async delete(c) {
        try {
            ...
            if (!user)
                return c.json(ResFormmater.failed("Unauthorized", 403), 403);
            const id = Number(c.req.param("id"));
            if (!id)
                return c.json(ResFormmater.failed("Id tidak ditemukan atau tidak valid"));
            const record = await PostsModel.getById(id);
