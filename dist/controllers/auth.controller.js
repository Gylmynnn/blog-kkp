...
export class AuthController {
    static async register(c) {
        try {
            ...
            if (!emailRegex.test(email))
                return c.json(ResFormmater.failed("Format email tidak valid", 400), 400);
            const existingUser = await UserModel.findByEmail(email);
            if (existingUser)
                return c.json(ResFormmater.failed("Email sudah digunakan", 400), 400);
    ...
    static async login(c) {
        try {
            ...
            if (!email || !password)
                return c.json(ResFormmater.failed("Email dan password wajib diisi", 400), 400);
            if (password.length < 6)
                return c.json(ResFormmater.failed("Password minimal 6 karakter"), 400);
            const user = await AuthModel.login(email);
            if (!user)
                return c.json(ResFormmater.failed("Tidak dapat menemukan akun dengan email tersebut. Silakan periksa kembali", 401), 401);
            const valid = await bcrypt.compare(password, user.password);
            if (!valid)
                return c.json(ResFormmater.failed("Kata sandi yang Anda masukkan tidak sesuai. Silakan coba lagi.", 401), 401);
            const payload = {
                userId: user.id,
                username: user.username,
                email: user.email,
